...
# New Ideas
## Feature selection
First off, one straightforward but clever idea was to start with a basic feature selection by looking at how the different features are distributed. The main focus was on picking features where the distribution pattern showed a clear difference between fraud and non-fraud cases. This way, only the most relevant features were chosen as inputs for the model.

## Hybrid Resampling
I noticed that most research sticks to just one resampling technique, but I decided to take a hybrid approach. I first used undersampling to get rid of a lot of noise and then oversampling to create a balanced but not equal number of classes. The key was not to match the majority and minority classes exactly, as that could have added unnecessary noise by generating too many minority samples.
...
1. **Best sampling strategy:**
   - **NearMiss combined with CTGAN (Strategy 3** is the best performing strategy. It shows the best balance of Precision, Recall, and F1 scores.CTGAN improves the model's performance on the minority class by generating a more realistic sample of the minority class, which may help the model learn the features of the minority class better.

2. **Best models:**
   - **XGBoost and LightGBM** perform the best in NearMiss combined with CTGAN (Strategy 3), with F1 scores of 0.85 for class 1. The XGBoost and LightGBM models are good at dealing with complex nonlinear relationships, and their tree structures can better capture patterns in the data, especially when dealing with unbalanced data , these two models are better able to cope with minority class classification tasks.

### Summary:
Combining the tabular data, the best sampling strategy is **NearMiss combined with CTGAN (strategy 3)**, while the best performing models are **XGBoost and LightGBM**. This combination allows for better identification of minority classes while striking a good balance between the individual model metrics.

The new samples generated by SMOTE-NC are mainly based on linear interpolation of existing minority class samples, which lacks diversity and therefore performs weakly when dealing with complex feature interactions and category variables, which can easily lead to overfitting. CTGAN, on the other hand, through generative adversarial network (GAN) technology, is able to capture the depth distribution of the data and generate more diverse and realistic samples, which are especially more effective in dealing with category variables and complex feature relationships, thus providing better model performance and generalisation ability on unbalanced data. This makes CTGAN outperform SMOTE-NC when balancing a small number of classes.
